...
public class TypegifView extends View implements Runnable {
	...
	public TypegifView(Context context, AttributeSet attrs) {
		...
		for (int i = 0; i < n; i++) {
			...
			switch (attr) {
			case R.styleable.gifView_gifView_src:
				int id = ta.getResourceId(R.styleable.gifView_gifView_src, 0);
				setSrc(id);
				break;

			case R.styleable.gifView_gifView_delay:
				int idelta = ta
						.getInteger(R.styleable.gifView_gifView_delay, 1);
				setDelta(idelta);
				break;

			case R.styleable.gifView_gifView_stop:
				boolean sp = ta.getBoolean(R.styleable.gifView_gifView_stop,
						false);
				if (!sp) {
					setStop();
				}
