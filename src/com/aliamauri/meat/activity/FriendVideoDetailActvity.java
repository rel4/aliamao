...
import com.aliamauri.meat.weight.RequestVideo_dialog;
import com.handmark.pulltorefresh.library.PullToRefreshBase;
import com.handmark.pulltorefresh.library.PullToRefreshBase.Mode;
import com.handmark.pulltorefresh.library.PullToRefreshBase.OnRefreshListener2;
import com.handmark.pulltorefresh.library.PullToRefreshListView;

public class FriendVideoDetailActvity extends BaseActivity implements OnClickListener {
	public final static String TAG = "FriendVideoDetailActvity";
	...
	protected View getRootView() {
		mId = getIntent().getStringExtra(GlobalConstant.INTENT_ID);
		mHXId = getIntent().getStringExtra(GlobalConstant.HXUSER_ID);
	...
	protected void initNet() {
		...
		getHttpHelp().sendGet(NetworkConfig.getDarenDeital(mId),
				DoyenBean.class, new MyRequestCallBack<DoyenBean>() {
					...
					public void onSucceed(DoyenBean bean) {
						...
						if ("1".equals(bean.status)) {
							if(bean.cont.hotList != null && bean.cont.hotList.size()<=0 ){
							 new RequestVideo_dialog(mActivity, mHXId);
							}
							dBean = bean;
							initData(bean);
							initLVData();
	...
	protected void initData(DoyenBean bean) {
		if (PrefUtils.getString(GlobalConstant.USER_ID, "").equals(mId)) {
	...
	private void loadMode() {
		getHttpHelp().sendGet(NetworkConfig.getUserHotVideo(mId, page),
			...
			if (position == 0) {
				if (historyAdapter == null) {
					...
					viewHolder.ll_idmt_myfansnum
							.setOnClickListener(new MyOnclick(mId, 1));
					viewHolder.ll_idmt_attentionminum
							.setOnClickListener(new MyOnclick(mId, 0));
				} else {
	...
	class MyOnclick implements OnClickListener {
		...
		public void onClick(View v) {
			Intent i;
			if (type == 2) {
				i = new Intent(FriendVideoDetailActvity.this, PlayActivity.class);
				i.putExtra(GlobalConstant.PLAY_VIDEO_ID, id);
				startActivity(i);
			} else {
				i = new Intent(FriendVideoDetailActvity.this, AttentionActivity.class);
	...
	public void onClick(View v) {
		...
		case R.id.rl_add_addfriend:
			if (!rl_add_addfriend.isSelected()) {
				CmdManager.getInstance().addContact(mId, "",
		...
		case R.id.rl_add_addattention:
			if (!rl_add_addattention.isSelected()) {
				getHttpHelp().sendGet(NetworkConfig.getgoFollow(mId),
				...
				getHttpHelp().sendGet(NetworkConfig.getdelfollows(mId),
						BaseBaen.class, new MyRequestCallBack<BaseBaen>() {
