...
public class VoiceCallActivity extends CallActivity implements OnClickListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		SDKHelper.getInstance().isVoiceCalling = true;
		...
		User user = userDao.getContactInfo(username);
		
		String nick = user.getNick();
		nickTextView.setText(TextUtils.isEmpty(nick) ? user.getUserId() : nick);
		String nativeAvatar = user.getNativeAvatar();
		HttpHelp httpHelp = new HttpHelp();
		if (TextUtils.isEmpty(nativeAvatar)) {
			httpHelp.showImage(swing_card, nativeAvatar);
		}else {
			httpHelp.showImage(user.getAvatar(), swing_card);
		}
	...
	protected void onDestroy() {
		super.onDestroy();
		SDKHelper.getInstance().isVoiceCalling = false;
	}
