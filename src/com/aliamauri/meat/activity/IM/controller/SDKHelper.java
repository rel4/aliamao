 ...
 */
package com.aliamauri.meat.activity.IM.controller;
...
import com.aliamauri.meat.activity.IM.model.DefaultHXSDKModel;
 ...
 * 不过继承类需要根据要求求提供相应的函数，尤其是提供一个{@link SDKModel}. 所以请实现abstract protected
...
public abstract class SDKHelper {
	...
	private List<HXSyncListener> syncAppointmentListListeners;
	...
	public synchronized boolean onInit(Context context) {
		 ...
		 * 异步邀请监听
		 */
		syncAppointmentListListeners = new ArrayList<HXSyncListener>();
	 ...
	 * @return
	 */
	abstract protected SDKModel createModel();
	...
	public void logout(final boolean unbindDeviceToken,
		...
		setPassword(null);
		reset();
		if (PlayActivity.instace != null) {
			PlayActivity.instace.finish();
			PlayActivity.instace = null;
		}
	 ...
	 * @param listener
	 */
	public void addSyncAppointmentListListener(HXSyncListener listener) {
		if (listener == null) {
			return;
		}
		if (!syncAppointmentListListeners.contains(listener)) {
			syncAppointmentListListeners.add(listener);
		}
	}
	 ...
	 * @param listener
	 */
	public void removeSyncAppointmentListListener(HXSyncListener listener) {
		if (listener == null) {
			return;
		}
		if (syncAppointmentListListeners.contains(listener)) {
			syncAppointmentListListeners.remove(listener);
		}
	}
	...
	public void asyncFetchContactsFromServer(
			final EMValueCallBack<List<ContactInfo>> callback) {
		...
		help.sendGet(NetworkConfig.getContactFromService(),
	...
	public void notifyAppointmentSyncListener(boolean success) {
		for (HXSyncListener listener : syncAppointmentListListeners) {
			listener.onSyncSucess(success);
		}
	}
	...
	public void asyncFetchBlackListFromServer(
		...
		httpHelp.sendGet(NetworkConfig.getContactBlackListFromService(),
				ContactInfoBean.class,
				new MyRequestCallBack<ContactInfoBean>() {
	...
	public synchronized void notifyForRecevingEvents() {
		...
		LogUtil.e(this, "isContactsSyncedWithServer ="
	...
	private boolean isAlreadyNotified() {
		if (isContactsSyncedWithServer && isBlackListSyncedWithServer) {
			return true;
		}
		return false;
	}
