...
import com.aliamauri.meat.activity.TelsFriendActivity;
...
import com.aliamauri.meat.activity.IM.activity.NewFriendsMsgActivity;
...
public class ContactlistFragment extends Fragment {
	...
	AppointmentListSyncListener appointmentSyncListener;
	View progressBar;
	Handler handler = new Handler();
	private User toBeProcessUser;
	private String toBeProcessUsername;

	class HXContactSyncListener implements SDKHelper.HXSyncListener {
		@Override
	...
	class HXBlackListSyncListener implements HXSyncListener {
		...
		public void onSyncSucess(boolean success) {
			getActivity().runOnUiThread(new Runnable() {
				...
				@Override
				public void run() {
					blackList = BlackListManager.getInstance()
							.getBlackListHXid();
					refresh();
				}
	...
	class AppointmentListSyncListener implements HXSyncListener {
		...
		public void onSyncSucess(boolean success) {
			getActivity().runOnUiThread(new Runnable() {
				...
				@Override
				public void run() {
					appointmentList = AppointmentManager.getInstance()
							.getAppointmentsHXIDList();
					refresh();
				}
	...
	class HXContactInfoSyncListener implements SDKHelper.HXSyncListener {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		// 黑名单列表
		blackList = BlackListManager.getInstance().getBlackListHXid();
		appointmentList = AppointmentManager.getInstance()
				.getAppointmentsHXIDList();
		...
		listView.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> parent, View view,
				...
				if (Constant.NEW_FRIENDS_USERNAME.equals(username)) {
					// 进入申请与通知页面
					User user = ((MySDKHelper) SDKHelper.getInstance())
							.getContactList()
				...
				} else if (Constant.OFFICIAL_USER.equals(username)) {
					// // 进入官方
					// startActivity(new Intent(getActivity(),
					// PublicChatRoomsActivity.class));
					MessageUtils.sendMyMessage(Constant.OFFICIAL_USER, MyApplication.getInstance().getUserName(),"欢迎使用阿里阿猫"+System.currentTimeMillis());
				} else if (Constant.APPOINTMENT_INVITATION.equals(username)) {
					User user = ((MySDKHelper) SDKHelper.getInstance())
							.getContactList().get(
									Constant.APPOINTMENT_INVITATION);
					user.setAppointmentMsgCount(0);
					Intent intent = new Intent(getActivity(),
							TelsFriendActivity.class);
					// NoticeBus bus = new NoticeBus();
					// bus.setMsg("hello");
					...
					// adapter.getItem(position).getNick())
					// .putExtra(GlobalConstant.FLAG_CHAT_TYPE,
					// ChatActivity.CHATTYPE_SINGLE));
					startActivity(new Intent(getActivity(),
							OtherDataActivity.class).putExtra(
							GlobalConstant.COMMENT_ADD_FRIEND,
		...
		SDKHelper.getInstance().addSyncContactListener(contactSyncListener);

		blackListSyncListener = new HXBlackListSyncListener();
		SDKHelper.getInstance().addSyncBlackListListener(blackListSyncListener);
		// 邀请监听
		appointmentSyncListener = new AppointmentListSyncListener();
		SDKHelper.getInstance().addSyncAppointmentListListener(
	...
	public void deleteContact(final User tobeDeleteUser) {
		...
		CmdManager.getInstance().deleteContact(tobeDeleteUser.getUserId(),
				new CmdManagerCallBack() {
					...
					public void onState(boolean isSucceed) {
						if (isSucceed) {
							...
							pd.dismiss();
							UIUtils.showToast(getActivity(), "删除好友成功");

						} else {
							UIUtils.showToast(getActivity(), st2);
							pd.dismiss();
	...
	private void moveToBlacklist(final String username) {
		...
		Map<String, User> allHXUsersMap = ContactManager.getInstance()
				.getAllHXUsersMap();
		if (!allHXUsersMap.containsKey(username)) {
			return;
		}
		CmdManager.getInstance().requsetServiceToBlackList(
				allHXUsersMap.get(username).getUserId(),
				new CmdManagerCallBack() {
	...
	public void onDestroy() {
		if (contactSyncListener != null) {
			SDKHelper.getInstance().removeSyncContactListener(
					contactSyncListener);
			contactSyncListener = null;
		}

		if (blackListSyncListener != null) {
			SDKHelper.getInstance().removeSyncBlackListListener(
					blackListSyncListener);
		}

		if (appointmentSyncListener != null) {
			SDKHelper.getInstance().removeSyncAppointmentListListener(
					appointmentSyncListener);
		}
	...
	private void getContactList() {
		...
		while (iterator.hasNext()) {
			...
			if (!Constant.NEW_FRIENDS_USERNAME.equals(entry.getKey())
					...
					&& !Constant.OFFICIAL_USER.equals(entry.getKey())
					&& !blackList.contains(entry.getKey())
					&& !appointmentList.contains(entry.getKey()))
		...
		// 官方
		if (users.get(Constant.OFFICIAL_USER) != null)
			contactList.add(0, users.get(Constant.OFFICIAL_USER));
		// 添加约会邀请
		if (users.get(Constant.APPOINTMENT_INVITATION) != null)
			contactList.add(0, users.get(Constant.APPOINTMENT_INVITATION));
		// 加入"群聊"和"聊天室"
